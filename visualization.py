...
def _get_nr_of_runs():
    return len(_get_analysis_results()["link_overlap"]["on_runs"])
...
def plot_link_overlap():
    ...
    plt.ylim([1, 30])
    plt.xlim([1, _get_nr_of_runs() + 1])
    title = "Overlap of link on HackerNews and DataTau frontpages"
    plt.title(title)
    # plt.show()
...
def _plot_trivia(data_key):
    ...
    if data_key == "nr_of_comments":
        ...
        ylabel = "Average post age"
        xlabel = "Run number"
        ylim = [0, max(data_dt + data_hn)]
        xlim = [1, _get_nr_of_runs()]
